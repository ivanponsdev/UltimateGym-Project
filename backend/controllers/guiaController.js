...
const crear = async (req, res) => {
  try {
    ...
    const nuevaGuia = new Guia({
      ...
      descripcion,
      objetivo,
      archivoUrl: req.file.path.replace(/\\/g, '/').replace(/^backend\//, ''),
...
const actualizar = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      if (guia.archivoUrl) {
        // Reconstruir ruta del sistema de archivos para eliminar
        const oldPath = guia.archivoUrl.startsWith('backend/') ? guia.archivoUrl : 'backend/' + guia.archivoUrl;
        if (fs.existsSync(oldPath)) {
          fs.unlinkSync(oldPath);
        }
      }
      guia.archivoUrl = req.file.path.replace(/\\/g, '/').replace(/^backend\//, '');
    }
...
const eliminar = async (req, res) => {
  try {
    ...
    if (guia.archivoUrl) {
      const filePath = guia.archivoUrl.startsWith('backend/') ? guia.archivoUrl : 'backend/' + guia.archivoUrl;
