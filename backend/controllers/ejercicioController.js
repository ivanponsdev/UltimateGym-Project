...
const obtenerPorId = async (req, res) => {
  try {
    ...
    console.log('Ejercicio obtenido:', {
      nombre: ejercicio.nombre,
      imagenTecnica: ejercicio.imagenTecnica,
      imagenTecnicaBool: !!ejercicio.imagenTecnica
    });
...
const crear = async (req, res) => {
  try {
    ...
    let imagenTecnica = '';
    if (req.file) {
      imagenTecnica = req.file.path.replace(/\\/g, '/').replace(/^backend\//, '')
      console.log('Archivo subido:', req.file.filename, 'Ruta guardada:', imagenTecnica);
    }
  ...
  try {
    ...
    if (req.file) {
      ...
      if (ejercicio.imagenTecnica) {
        const rutaImagenAnterior = ejercicio.imagenTecnica.startsWith('uploads/') ? ejercicio.imagenTecnica : 'backend/' + ejercicio.imagenTecnica
        if (fs.existsSync(rutaImagenAnterior)) {
          fs.unlinkSync(rutaImagenAnterior)
          console.log('Imagen anterior eliminada:', rutaImagenAnterior)
        }
      }
      ejercicio.imagenTecnica = req.file.path.replace(/\\/g, '/').replace(/^backend\//, '')
      console.log('Nuevo archivo subido:', req.file.filename, 'Ruta guardada:', ejercicio.imagenTecnica);
    }
...
const eliminar = async (req, res) => {
  try {
    ...
    if (ejercicio.imagenTecnica) {
      const rutaImagen = ejercicio.imagenTecnica.startsWith('uploads/') ? ejercicio.imagenTecnica : 'backend/' + ejercicio.imagenTecnica;
