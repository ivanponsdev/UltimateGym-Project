...
exports.obtenerEstadisticasGlobales = async (req, res) => {
  try {
    ...
    // Total interacciones con Paco
    const interaccionesLandBot = await InteraccionPaco.countDocuments()
...
// Registrar una interacción con el asistente Paco
exports.registrarInteraccionPaco = async (req, res) => {
  try {
    const userId = req.user.id
    await InteraccionPaco.create({ usuario: userId, tipo: 'apertura' })
    res.json({ mensaje: 'Interacción registrada' })
  } catch (error) {
    console.error('Error al registrar interacción con Paco:', error)
    res.status(500).json({ mensaje: 'Error al registrar interacción', error: error.message })
  }
}
